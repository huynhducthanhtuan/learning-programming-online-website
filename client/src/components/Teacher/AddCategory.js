        ...
        return (
         <form >
             <div className='form-group'>
                 ...
                 <input type='text' value={name} className='form-control' onChange={handleChange('name')} autoFocus required/>
             </div>
             <button className='btn btn-outline-primary mb-4' onClick={submitForm}>Create Category</button>
         </form>
        )
     }   
    ...
    return (
        <Layout title="Add a new Category" description={`${user.name} Ready to add a new Category`} >
            <div className="container col-md-3 ">
                ...
                {successShow()}
                {errorShow()}
                <Link to="/admin/dashboard" className='text-warning'>Go dashboard</Link>
            </div> 
        </Layout>
    )
}
